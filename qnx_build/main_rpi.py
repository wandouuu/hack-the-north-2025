...
import keyboard
import numpy as np
from video_transmission import frame_server

# ---------- Config ----------
PINCH_THRESHOLD = 25   # px, strict pinch
SMOOTHING_WINDOW = 5
MIN_POINT_DIST = 12
CAM_INDEX = 0
FLIP_FRAME = True
        ...
        while self._running:
            ...
            if self.hand_raised:
                ...
                if results.multi_hand_landmarks:
                    ...
                    print("The pinch distance is: ", pinch_dist)
...
def main():
    ...
    pose = mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7, model_complexity=0)
    mp_hands = mp.solutions.hands
    hands = mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.6, min_tracking_confidence=0.6)

    # CONSTANTS
    FINGERTIP_DIST_THRESH = 45
    SWIPE_THRESH = 80                 # reduced threshold for easier swipes
    FINGERTIP_DIST_THRESH = 45        # closer fingers for more reliable detection
    swipe_cooldown = 0.4              # reduced cooldown
    swipe_time_limit = 1.2            # longer time window
    MIN_SWIPE_SPEED = 50              # minimum speed threshold


    frame_gen = frame_server()  # Use the generator
    ...
    while True:
        ...
        if hand_is_raised and hand_results.multi_hand_landmarks:
            ...
            ix = int(lm.landmark[8].x * cam_w) # i for index
            iy = int(lm.landmark[8].y * cam_h)
            tx = int(lm.landmark[4].x * cam_w) # t for thumb
            ty = int(lm.landmark[4].y * cam_h)
            pinch_dist = math.hypot(ix - tx, iy - ty)
            is_pinched = pinch_dist < PINCH_THRESHOLD
            ...
            for i in range(len(fingertip_ids)):
                fx = int(lm.landmark[fingertip_ids[i]].x * cam_w)
                fy = int(lm.landmark[fingertip_ids[i]].y * cam_h)
                if i == 4:
                    next_x = int(lm.landmark[fingertip_ids[0]].x * cam_w)
                    next_y = int(lm.landmark[fingertip_ids[0]].y * cam_h)
                else:
                    next_x = int(lm.landmark[fingertip_ids[i+1]].x * cam_w)
                    next_y = int(lm.landmark[fingertip_ids[i+1]].y * cam_h)
                dist = math.hypot(fx - next_x, fy - next_y)
                if dist >= 30:
                    is_fist = False
                    break
            overlay.on_point(ix, iy, is_pinched, is_fist)
        else:
            overlay.on_point(-1, -1, False, False)
